$(document).ready(function() {
  ...
  $('.instractor-detils-area .button').on('click', function() {
      var filterValue = $(this).attr('data-filter');
      
      $('.instractor-detils-area .button').removeClass('is-checked');
          ...
          $(window).on("load", function() {
            ...
            if(isotope.length){
                var iso = new Isotope( '.filter', {
                    ...
                    // تحديد خاصية الاتجاه من اليمين لليسار في التوزيع
                    originLeft: false, // وضع العناصر من اليمين إلى اليسار
                });
...
$(document).ready(function () {
  ...
  $('.del-popup').on('click', function (e) {
      ...
      var text = "تم حذف الكورس بنجاح!";
      var i = 0;
      var speed = 100;
  
      var interval = setInterval(function () {
          $('#success-text').append(text[i]);
          i++;
          if (i === text.length) {
...
player.on('click', function(event) {
    if (event.target.classList.contains('vjs-download')) {
        event.preventDefault();
    }
});
